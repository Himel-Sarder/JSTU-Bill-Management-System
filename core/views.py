...
def bill_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if action == 'send':
                bill.status = 'pending'  # Send button sets status to pending
...
def get_work_types_by_degree(request):
    """Get work types filtered by degree type"""
    ...
    # Return both the actual name and the display name
    work_type_list = []
    for wt in work_types:
        work_type_list.append({
            'value': wt.name,  # Store the actual name
            'display': str(wt),  # Store the display name with degree
...
def get_benefit_choices_by_degree(request):
    """Get benefit choices filtered by work type and degree"""
    work_type_name = request.GET.get('work_type')
    degree_type = request.GET.get('degree_type', 'honors')

    if work_type_name:
        try:
            # Find the work type with matching name and degree_type
        ...
        except WorkType.MultipleObjectsReturned:
            # If multiple found (shouldn't happen with unique_together), get the one with matching degree
...
def get_work_type_amount(request):
    """Get amount for work type (when no benefit needed)"""
    work_type_name = request.GET.get('work_type')
    degree_type = request.GET.get('degree_type', 'honors')
...
def bill_status(request):
    """View for displaying bill status with filtering - only shows sent bills (not draft)"""
    # Get all bills for the current user (or all bills for admins) EXCLUDING draft
    if request.user.profile.user_type in ['চেয়ারম্যান', 'অফিস সহকারী']:
        # Admins see all non-draft bills
        bills_list = Bill.objects.exclude(status='draft').order_by('-created_at')
    else:
        # Regular users see only their own non-draft bills
        bills_list = Bill.objects.filter(user=request.user).exclude(status='draft').order_by('-created_at')

    # Get counts for each status (only for non-draft bills)
    ...
    # For regular users, only count their own bills
    if request.user.profile.user_type not in ['চেয়ারম্যান', 'অফিস সহকারী']:
...
def send_bill(request, bill_id):
    ...
    messages.success(request, 'বিল সফলভাবে পাঠানো হয়েছে! স্ট্যাটাস পৃষ্ঠায় দেখুন।')
